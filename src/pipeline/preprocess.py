...
df_sales = standardize_headers(
    spark.read.csv(f"{HDFS_RAW}/sales_202*.csv", header=True, encoding="utf-8")) \
...
df_stores = standardize_headers(
    spark.read.csv(f"{HDFS_RAW}/stores_202*.csv", header=True, encoding="utf-8")) \
    .select("기준_년분기_코드", "상권_코드", "서비스_업종_코드_명",
...
df_foot = standardize_headers(
    spark.read.csv(f"{HDFS_RAW}/foot_traffic.csv", header=True, encoding="utf-8")) \
