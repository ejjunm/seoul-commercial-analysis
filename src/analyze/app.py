...
with tab3:
    ...
    with sa:
        ...
        st.caption("5-Fold CV AUC 0.548 (±0.026) · 불균형 보정 적용 · 안착비율 38%")

        shap_df = pd.DataFrame({
            "변수": ["MZ 매출 비중", "시장 활력도", "매출 변동계수", "객단가", "업종 특화도"],
            "SHAP": [0.200, 0.180, 0.159, 0.135, 0.130],
        }).sort_values("SHAP")
        ...
        st.info(
            ...
            "AUC 0.548은 공공데이터 한계(임대료·개인역량 부재)를 반영하며, "
            "모델은 예측이 아닌 패턴 기반 학습 도구로 활용."
        )

    with sb:
        st.subheader(f"라이징 추천점수 Top 15")
        st.caption("안착 적합도 × 진입 가능성(경쟁밀도·유동인구 적정도) 복합 점수")
