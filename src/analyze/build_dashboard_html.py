...
def load_geo():
    ...
    print(f"GeoJSON 로컬 파일 없음 → 다운로드: {GEO_URL}")
    with urllib.request.urlopen(GEO_URL) as r:
        data = json.loads(r.read().decode())
    with open(GEO_LOCAL, "w", encoding="utf-8") as wf:
...
shap_df = pd.DataFrame({
    "변수": ["MZ 매출 비중", "시장 활력도", "매출 변동계수", "객단가", "특화도"],
    "SHAP": [0.200, 0.2180, 0.160, 0.135, 0.130],
}).sort_values("SHAP")
...
<div class="header">
  ...
  <p>2024~2025년 8분기 · 외식·소매 · Q1 거시 → Q2 핫플 → Q3 청년 골목</p>
</div>
<div class="tabs">
...
<div class="tab-content" id="tab2">
  ...
  <div class="two-col">
    <div class="section"><h3>XGBoost SHAP 변수 중요도</h3>{fig_div(fig_shap)}
      <div class="note">MZ 매출 비중·시장 활력도가 핵심 신호. AUC 0.548는 공공데이터 한계를 반영하며 검토 목적으로 활용.</div></div>
